...
from ..helpers.doctorconnect import contact_doctor
from ..helpers.ambulance_dispatch import request_ambulance

from ..models import db, User, UserMessage, ResponseMessage, ChatSession, ChatMemory

# ✅ Load .env (important for Twilio, OpenAI, etc.)
...
def whatsapp_webhook():
    ...
    if new_user:
        greeting = (
            "👋 Hi there! Welcome to MediLink — your all-in-one health assistant.\n\n"
            "I can help you with:\n"
            "1️⃣ Check symptoms\n"
            "2️⃣ Find nearby clinics\n"
            "3️⃣ Upload prescription\n"
            "4️⃣ Contact a doctor\n"
            "5️⃣ Request an ambulance\n"
            "6️⃣ Get health tips\n\n"
            "👉 Reply with a number to continue."
        )

        log_chat(user_phone, greeting, "bot")
    ...
    if session.session_state == "main_menu":
        ...
        if normalized in greetings:
            ai_reply = (
                "👋 Hello! Welcome back to MediLink.\n\n"
                "I can help you with:\n"
                "1️⃣ Check symptoms\n"
                "2️⃣ Find nearby clinics\n"
                "3️⃣ Upload prescription\n"
            ...
            db.session.commit()
            ai_reply = "📍 Please share your location or town name to find nearby clinics."

        elif normalized == "3":
            ai_reply = "📸 Please upload a clear photo of your prescription."

        elif normalized == "4":
            ai_reply = contact_doctor(user_phone)

        elif normalized == "5":
            ai_reply = request_ambulance(user_phone)
        ...
        elif normalized in ["0", "menu", "help"]:
            ai_reply = (
    ...
    # --- 7️⃣ Handle Symptom Checker ---
        ...
        if normalized in ["menu", "0", "back"]:
            ...
            db.session.commit()
            ai_reply = "🔙 Back to main menu — reply with a number."
        else:
